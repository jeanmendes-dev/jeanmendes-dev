...
# A agregação acontece ANTES de qualquer join, eliminando a fonte
# dos produtos cartesianos que geravam AVISITN decimais.
...
merge_ard <- function(spine, extracted_all) {
  ...
  for (ds in extracted_all) {
    for (obj in ds$extracted) {
      ...
      if (all(c("USUBJID", "AVISITN") %in% names(obj))) {
        ...
        # Garantir grain do objeto antes do join
  ...
  decimal_check <- ard %>% filter(AVISITN != as.integer(AVISITN))
  if (nrow(decimal_check) > 0) {
    log_warn(paste("AVISITNs decimais no ARD final:", nrow(decimal_check), "linhas"))
  }

  # Garantir ordem de colunas: USUBJID | AVISITN | AVISIT | restante

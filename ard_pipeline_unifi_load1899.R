...
OUTPUT_ARD      <- "/mnt/unifi_load1899_ml_dataset_v1.csv"
OUTPUT_COVERAGE <- "/mnt/unifi_load1899_ml_coverage_report_v1.csv"

# ---------------------------------------------------------------
# Datasets confirmados no diagnóstico UNIFI load-1899
# Nota: arquivos com sufixo .sas7bdat — pipeline normaliza para
# lowercase sem extensão nos prefixos de coluna.
# ---------------------------------------------------------------
EXPECTED_DATASETS <- c(
  "adae.sas7bdat",
  "adbdc.sas7bdat",
...
# ---------------------------------------------------------------
# Datasets SEM AVISITN confirmados no diagnóstico
#   adae.sas7bdat    — sem AVISITN; tem ASTDT/ASTDY
#   adbdc.sas7bdat   — sem AVISITN; tem ASTDT (cross-sectional baseline)
#   adcm.sas7bdat    — sem AVISITN; cross-sectional de medicações
#   adcort.sas7bdat  — sem AVISITN; tem ADT/ADY (corticosteroid daily)
#   addeath.sas7bdat — sem AVISITN; 2 linhas apenas
#   adds.sas7bdat    — sem AVISITN; disposição de sujeitos
#   addv.sas7bdat    — sem AVISITN; tem DVSTDT
#   adeff.sas7bdat   — sem AVISITN; tabela de eficácia cross-sectional
#   adhist.sas7bdat  — sem AVISITN; histórico de hospitalizações
#   adhist1.sas7bdat — sem AVISITN; histórico de histologia
#   adhist2.sas7bdat — sem AVISITN; histórico de histologia detalhada
#   adho.sas7bdat    — sem AVISITN; tem ASTDT
#   admh.sas7bdat    — sem AVISITN; histórico médico
#   adsg.sas7bdat    — sem AVISITN; tem ASTDT
#   adsl.sas7bdat    — sem AVISITN; 1 linha por sujeito
#   adtte.sas7bdat   — sem AVISITN; time-to-event
# ---------------------------------------------------------------

# ---------------------------------------------------------------
# Particularidades do UNIFI vs GALAXI
#
# 1. ESTRUTURA DE TRATAMENTO MULTI-PERÍODO
#    UNIFI tem 3 períodos (TRT01P, TRT02P, TRT03P) com subgrupos
#    (TR01PG1, TR02PG1, TR02PG2). GALAXI tinha apenas TRT01P/TRT02P.
#
# 2. FLAGS DE ANÁLISE DISTINTOS
#    UNIFI usa: SAFFL, SAFW8FL, SAF2FL, COMPLFL, COMP20FL,
#               ENTERMFL, ENEXTFL, RANDFL, RAND2FL
#    (vs FASFL/SAFFL/USMFL do GALAXI-1/2 e ATASFL/ATSAFFL do GALAXI-3)
#
# 3. ÂNCORA DA SPINE — ADMAYO
#    admayo.sas7bdat: 790.256 linhas — maior dataset do estudo,
#    cobre endpoint central UC (Mayo Score: SFSCORE, RBSCORE,
#    ENSCORE, PMAYO, MMAYO, MAYO, RESP). Âncora natural para UC,
#    equivalente ao advscdai.sas7bdat nos estudos de Crohn.
#
# 4. DATASETS EXCLUSIVOS DO UNIFI
#    adchem   — labs de química (ALB, ALT, AST, CREAT, etc.)
#    adhema   — hematologia (WBC, HGB, PLAT, etc.)
#    aduceis  — UCEIS score (endoscopia UC)
#    adsf36   — SF-36 quality of life
#    adecon   — desfechos econômicos (WPAI)
#    adtfi/adtfm — treatment failure induction/maintenance
#    adsg     — cirurgias
# ---------------------------------------------------------------

ANALYSIS_FLAGS_UNIFI <- c(
...
# 3. BUILD SPINE
# ===============================
# Âncora primária: admayo.sas7bdat (790.256 linhas)
# Endpoints centrais UC: SFSCORE, RBSCORE, ENSCORE, PMAYO, MMAYO,
# MAYO, RESP — equivalente ao advscdai nos estudos de Crohn.
# Complementada com adsl.sas7bdat para garantir todos os 1.331 sujeitos.
build_spine <- function(datasets) {

  spine_parts <- list()

  anchor <- "admayo.sas7bdat"
  if (anchor %in% names(datasets)) {
    ...
    log_warn("Âncora admayo.sas7bdat não encontrada — spine construída de todos os datasets")
  }
...
run_pipeline <- function() {
  ...
  log_info("=== UNIFI load-1899 ARD PIPELINE INICIADO ===")
  ...
  # 3. Spine baseada em admayo.sas7bdat + adsl.sas7bdat

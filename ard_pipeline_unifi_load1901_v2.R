...
build_spine <- function(datasets) {
  ...
  if (anchor %in% names(datasets)) {
    spine_parts[[anchor]] <- unique(
      datasets[[anchor]][, .(USUBJID, AVISIT, AVISITN)]
    )
    log_info("Using admayo2.sas7bdat as primary spine anchor")
  } else {
    log_warn("admayo2.sas7bdat not found; building spine from all datasets with AVISITN")
  }
...
run_pipeline <- function() {
  ...
  log_info("=== UNIFI load-1901 ARD PIPELINE STARTED ===")
